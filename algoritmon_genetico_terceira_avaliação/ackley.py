...
def ackley_function(x):
    n = 4
    primeira_soma = np.sum(x**2) / n
    segunda_soma = np.sum(np.cos(2 * math.pi * x)) / n
    return -20 * math.exp(-0.2 * math.sqrt(primeira_soma)) - math.exp(segunda_soma) + 20 + math.e
...
class Cromossomo:
    def __init__(self, genes):
        self.genes = genes
        self.aptidao = ackley_function(genes)

def gerar_cromossomo(tam):
    genes = np.random.uniform(-30, 30, size=tam)
...
def mutacao(cromossomo):
    ...
    for i in range(len(genes)):
        ...
        if random.uniform(0, 1) <= 0.02:
            genes[i] = np.random.uniform(-30, 30)
    return Cromossomo(genes)
