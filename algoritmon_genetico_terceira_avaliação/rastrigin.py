...
def rastrigin_function(x):
    return A*n + np.sum(x**2 - A*np.cos(2*np.pi*x) + 10)
...
class Cromossomo:
    def __init__(self, genes):
        self.genes = genes
        self.aptidao = rastrigin_function(genes)

def gerar_cromossomo(tam):
    genes = np.random.uniform(-5.12, 5.12, size=tam)
...
def mutacao(cromossomo):
    ...
    for i in range(len(genes)):
        ...
        if random.uniform(0, 1) <= 0.02:
            genes[i] = np.random.uniform(-5.12, 5.12)
    return Cromossomo(genes)
