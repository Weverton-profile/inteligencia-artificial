...
def algoritmo_genetico(tamanho_populacao, geracoes):
    populacao = gerar_populacao(tamanho_populacao, 2)
...
class Cromossomo:
    def __init__(self, genes):
        self.genes = genes
        self.aptidao = easom_function(genes)

def gerar_cromossomo(tam):
    genes = np.random.uniform(-100, 100, size=tam)
...
def mutacao(cromossomo):
    ...
    for i in range(len(genes)):
        ...
        if random.uniform(0, 1) <= 0.02:
            genes[i] = np.random.uniform(-100, 100)
    return Cromossomo(genes)
